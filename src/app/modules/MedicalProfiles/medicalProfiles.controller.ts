...
const createMedicalProfile = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { ...medicalProfileData } = req.body;
        const medicalProfile = await medicalProfileServices.createMedicalProfile(medicalProfileData);
        res.status(200).json({
            status: 'success',
